...
namespace startdemos_ui.Utils
{
    public class SignatureScanner
    {
        ...
        public IntPtr FindStringRef(string input)
        {
            ...
            {
                SigScanTarget newTarg = new SigScanTarget(0, $"68 {GetByteArray(f_ptr.ToInt32())}");
                return f_scanner.Scan(newTarg);
            };
            return Scan(target);
        }

        public IntPtr ReadCall(IntPtr ptr)
        {
            if (ptr == IntPtr.Zero)
                return IntPtr.Zero;
            return (IntPtr)(_process.ReadValue<int>(ptr + 0x1) + (int)(ptr + 5));
        }
