...
namespace startdemos_plus.UI
{
    public partial class DemoPlayForm : UserControl
    {
        ...
        public DemoPlayForm()
        {
            ...
            Settings.AddSetting(
                "playing-auto_play",
                x => { if (bool.TryParse(x, out var a)) chkAutoPlayNext.Checked = a; },
        ...
        private void butPlayPause_Click(object sender, EventArgs e)
        {
            ...
            if (!_queuePlaying)
            {
                _player.Begin(_queuer.GetPlayed(), (int)nudWaitTime.Value, chkAutoPlayNext.Checked, boxCommands.Text);
                _queuePlaying = true;
                butPlayPause.SetState("pause");
